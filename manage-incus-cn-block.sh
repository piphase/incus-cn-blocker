...
ENV_ROUTE_V4_URL="${ROUTE_V4_URL-}"
ENV_ROUTE_V6_URL="${ROUTE_V6_URL-}"
...
ROUTE_V4_URL="${ROUTE_V4_URL:-${ROUTE_URL:-https://raw.githubusercontent.com/misakaio/chnroutes2/master/chnroutes.txt}}"
ROUTE_V6_URL="${ROUTE_V6_URL:-https://ftp.apnic.net/stats/apnic/delegated-apnic-latest}"
TIMER_INTERVAL="${TIMER_INTERVAL:-12h}"
CACHE_MIN_V4_PREFIXES="${CACHE_MIN_V4_PREFIXES:-${CACHE_MIN_PREFIXES:-1000}}"
CACHE_MIN_V6_PREFIXES="${CACHE_MIN_V6_PREFIXES:-500}"
...
ROUTE_V4_FILE="$STATE_DIR/cn_ipv4.txt"
ROUTE_V6_FILE="$STATE_DIR/cn_ipv6.txt"
...
load_state() {
  ...
  if [[ -n "${ROUTE_URL:-}" && -z "$ENV_ROUTE_URL" && -z "$ENV_ROUTE_V4_URL" ]]; then
    ROUTE_V4_URL="$ROUTE_URL"
  fi
  ...
  if [[ -n "$ENV_ROUTE_URL" ]]; then
    ROUTE_V4_URL="$ENV_ROUTE_URL"
  fi

  if [[ -n "$ENV_ROUTE_V4_URL" ]]; then
    ROUTE_V4_URL="$ENV_ROUTE_V4_URL"
  fi

  if [[ -n "$ENV_ROUTE_V6_URL" ]]; then
    ROUTE_V6_URL="$ENV_ROUTE_V6_URL"
  fi
}
...
save_state() {
  ...
  local last_v4_source="${3:-${LAST_V4_SOURCE_URL:-$ROUTE_V4_URL}}"
  local last_v6_source="${4:-${LAST_V6_SOURCE_URL:-$ROUTE_V6_URL}}"
...

  {
    ...
    printf 'ROUTE_URL=%q\n' "$last_v4_source"
    printf 'ROUTE_V4_URL=%q\n' "$last_v4_source"
    printf 'ROUTE_V6_URL=%q\n' "$last_v6_source"
    printf 'LAST_V4_SOURCE_URL=%q\n' "$last_v4_source"
    printf 'LAST_V6_SOURCE_URL=%q\n' "$last_v6_source"
...
EOF
  ...
  cat >"$update_unit" <<EOF
[Unit]
Description=Refresh China IPv4 and IPv6 cache for Incus destination blocking
...
EOF
  ...
  cat >"$timer_unit" <<EOF
[Unit]
Description=Periodic China IPv4 and IPv6 refresh for Incus destination blocking
...
download_to_file() {
  ...
  if have_command curl; then
    ...
    curl "${curl_args[@]}" "$source_url" -o "$destination"
  else
    local wget_args=(--quiet --output-document="$destination")
...
fetch_routes_v4() {
  ...
  tmp_file="$(mktemp "$STATE_DIR/cn-routes-v4.XXXXXX")"
  raw_file="${tmp_file}.raw"

  download_to_file "$ROUTE_V4_URL" "$raw_file"
  grep -E '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/[0-9]+$' "$raw_file" | sort -u >"$tmp_file"
  rm -f "$raw_file"

  line_count="$(wc -l <"$tmp_file" | tr -d '[:space:]')"
  [[ -n "$line_count" ]] || die "无法统计下载到的 IPv4 前缀数量。"

  if (( line_count < CACHE_MIN_V4_PREFIXES )); then
    rm -f "$tmp_file"
    die "下载到的 IPv4 路由数据看起来不完整（仅 $line_count 条前缀），已保留旧缓存。"
  fi

  mv "$tmp_file" "$ROUTE_V4_FILE"
  log "已缓存 $line_count 条 IPv4 前缀，来源：$ROUTE_V4_URL"
}
...
fetch_routes_v6() {
  ...
  tmp_file="$(mktemp "$STATE_DIR/cn-routes-v6.XXXXXX")"
  raw_file="${tmp_file}.raw"

  download_to_file "$ROUTE_V6_URL" "$raw_file"
  awk -F'|' '$2=="CN" && $3=="ipv6" && ($7=="allocated" || $7=="assigned") { print $4 "/" $5 }' "$raw_file" | sort -u >"$tmp_file"
  rm -f "$raw_file"

  line_count="$(wc -l <"$tmp_file" | tr -d '[:space:]')"
  [[ -n "$line_count" ]] || die "无法统计下载到的 IPv6 前缀数量。"

  if (( line_count < CACHE_MIN_V6_PREFIXES )); then
    rm -f "$tmp_file"
    die "下载到的 IPv6 路由数据看起来不完整（仅 $line_count 条前缀），已保留旧缓存。"
  fi

  mv "$tmp_file" "$ROUTE_V6_FILE"
  log "已缓存 $line_count 条 IPv6 前缀，来源：$ROUTE_V6_URL"
}
...
fetch_routes() {
  ...
  LAST_V4_SOURCE_URL="$ROUTE_V4_URL"
  LAST_V6_SOURCE_URL="$ROUTE_V6_URL"
  save_state "${ENABLED:-0}" "$LAST_UPDATED_AT" "$LAST_V4_SOURCE_URL" "$LAST_V6_SOURCE_URL"
}

generate_payload() {
  local route_v4_file="$1"
  local route_v6_file="$2"
  local payload_file="$3"
  local has_v4=0
  local has_v6=0

  [[ -s "$route_v4_file" ]] && has_v4=1
  [[ -s "$route_v6_file" ]] && has_v6=1
  (( has_v4 == 1 || has_v6 == 1 )) || die "路由缓存为空，请先执行“安装/初始化”或“更新数据库”。"

  {
    echo "table $TABLE_FAMILY $TABLE_NAME {"

    if (( has_v4 == 1 )); then
      echo "    set $SET_V4_NAME {"
      echo "        type ipv4_addr"
      echo "        flags interval"
      echo "        elements = {"
      sed '$!s/$/,/' "$route_v4_file" | sed 's/^/            /'
      echo "        }"
      echo "    }"
      echo
    fi
    ...
    if (( has_v6 == 1 )); then
      echo "    set $SET_V6_NAME {"
      echo "        type ipv6_addr"
      echo "        flags interval"
      echo "        elements = {"
      sed '$!s/$/,/' "$route_v6_file" | sed 's/^/            /'
      echo "        }"
      echo "    }"
      echo
    fi
    ...
    if (( has_v4 == 1 )); then
      echo "        iifname \"$BRIDGE_NAME\" ip daddr @$SET_V4_NAME ct state new counter drop"
    fi
    if (( has_v6 == 1 )); then
      echo "        iifname \"$BRIDGE_NAME\" ip6 daddr @$SET_V6_NAME ct state new counter drop"
    fi
    echo "    }"
    echo "}"
  } >"$payload_file"
...
command_enable() {
  ...
  if (( refresh == 1 )) || [[ ! -s "$ROUTE_V4_FILE" ]] || [[ ! -s "$ROUTE_V6_FILE" ]]; then
    if (( use_cache_only == 1 )); then
      if [[ ! -s "$ROUTE_V4_FILE" && ! -s "$ROUTE_V6_FILE" ]]; then
        die "当前既没有 IPv4 缓存，也没有 IPv6 缓存，无法使用 --use-cache-only。"
      fi
      warn "当前缓存不完整，将仅按现有缓存应用规则。"
    else
      fetch_routes
    fi
  fi
  ...
  generate_payload "$ROUTE_V4_FILE" "$ROUTE_V6_FILE" "$payload_file"
  apply_payload "$payload_file"
  rm -f "$payload_file"

  ENABLED=1
  save_state "1" "${LAST_UPDATED_AT:-}" "${LAST_V4_SOURCE_URL:-$ROUTE_V4_URL}" "${LAST_V6_SOURCE_URL:-$ROUTE_V6_URL}"
  ...
  log "已启用拦截：来自 $BRIDGE_NAME 的新建 IPv4/IPv6 连接将按中国双栈前缀列表进行阻断。"
}
...
command_apply_cached() {
  ...
  [[ -s "$ROUTE_V4_FILE" || -s "$ROUTE_V6_FILE" ]] || die "当前没有可用的 IPv4/IPv6 路由缓存。"

  payload_file="$(mktemp "$STATE_DIR/payload.XXXXXX.nft")"
  generate_payload "$ROUTE_V4_FILE" "$ROUTE_V6_FILE" "$payload_file"
  apply_payload "$payload_file"
  rm -f "$payload_file"
  log "已将缓存中的双栈路由规则应用到 nftables。"
}
...
command_update() {
  ...
  if (( apply_always == 1 )) || (( apply_if_enabled == 1 && ${ENABLED:-0} == 1 )) || (( apply_if_enabled == 0 && ${ENABLED:-0} == 1 )); then
    ...
    apply_payload "$payload_file"
    rm -f "$payload_file"
    log "双栈数据库已更新，并已将最新规则应用到 nftables。"
  else
    log "双栈数据库已更新。当前未启用拦截，因此没有重新应用规则。"
  fi
}
...
command_disable() {
  ...
  disable_units
  ENABLED=0
  save_state "0" "${LAST_UPDATED_AT:-}" "${LAST_V4_SOURCE_URL:-$ROUTE_V4_URL}" "${LAST_V6_SOURCE_URL:-$ROUTE_V6_URL}"
  log "已关闭拦截，并停止相关定时任务。"
}

command_status() {
  local table_state="不存在"
  local cache_v4_state="不存在"
  local cache_v6_state="不存在"
  local timer_enabled="不可用"
  local timer_active="不可用"
  local apply_enabled="不可用"
  local prefix_v4_count="0"
  local prefix_v6_count="0"
  local saved_route_v4_url=""
  local saved_route_v6_url=""

  load_state
  saved_route_v4_url="${ROUTE_V4_URL:-}"
  saved_route_v6_url="${ROUTE_V6_URL:-}"

  if nft list table "$TABLE_FAMILY" "$TABLE_NAME" >/dev/null 2>&1; then
    table_state="已存在"
  fi

  if [[ -s "$ROUTE_V4_FILE" ]]; then
    cache_v4_state="已存在"
    prefix_v4_count="$(wc -l <"$ROUTE_V4_FILE" | tr -d '[:space:]')"
  fi

  if [[ -s "$ROUTE_V6_FILE" ]]; then
    cache_v6_state="已存在"
    prefix_v6_count="$(wc -l <"$ROUTE_V6_FILE" | tr -d '[:space:]')"
  fi
...
网桥名称：$BRIDGE_NAME
IPv4 数据源：${saved_route_v4_url}
IPv6 数据源：${saved_route_v6_url}
状态文件中的启用标记：${ENABLED:-0}
自定义表状态：$table_state
初始备份状态：$( [[ -f "$INITIAL_BACKUP" ]] && echo "已存在" || echo "不存在" )
IPv4 缓存状态：$cache_v4_state（$prefix_v4_count 条前缀）
IPv6 缓存状态：$cache_v6_state（$prefix_v6_count 条前缀）
...
命令：
  ...
  enable [options]         使用缓存或最新下载的中国 IPv4/IPv6 列表启用拦截
  disable                  关闭拦截，只删除自定义 nftables 表
  update [options]         更新中国 IPv4/IPv6 数据库，并在需要时重新应用规则
...
环境变量覆盖：
  ...
  ROUTE_URL                兼容别名，等同于 ROUTE_V4_URL
  ROUTE_V4_URL             默认值：$ROUTE_V4_URL
  ROUTE_V6_URL             默认值：$ROUTE_V6_URL
  CACHE_MIN_PREFIXES       兼容别名，等同于 CACHE_MIN_V4_PREFIXES
  CACHE_MIN_V4_PREFIXES    默认值：$CACHE_MIN_V4_PREFIXES
  CACHE_MIN_V6_PREFIXES    默认值：$CACHE_MIN_V6_PREFIXES
